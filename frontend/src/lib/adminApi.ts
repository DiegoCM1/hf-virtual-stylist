...
export async function listFabrics(params?: { q?: string; status?: "active" | "inactive"; limit?: number; offset?: number }) {
  ...
  // return r.json();
  const { data } = await api.get(`/admin/fabrics?${usp.toString()}`);
  return data;
}
...
export async function createFabric(body: {
  ...
  const { data } = await api.post("/admin/fabrics", body);
  return data;
}

// POST /admin/fabrics/:id/deactivate  (if you still use this route anywhere)
export async function deactivateFabric(id: number) {
  const { data } = await api.post(`/admin/fabrics/${id}/deactivate`);
  return data;
}

// PATCH /admin/fabrics/:id
export async function setFabricStatus(id: number, status: "active" | "inactive") {
  ...
  // return r.json();
  const { data } = await api.patch(`/admin/fabrics/${id}`, { status });
  return data;
}